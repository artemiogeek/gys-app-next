...
import LogisticaCotizacionSelectorModal from './LogisticaCotizacionSelectorModal'
import { 
  Trophy,
  AlertTriangle,
  CheckCircle2,
...
export default function LogisticaListaDetalleItemTableProfessional({ items, onUpdated }: Props) {
  // ✅ Ya no necesitamos estado de expansión, usamos modal directamente
        ...
        <table className="w-full text-sm border rounded-xl overflow-hidden shadow-sm">
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100 text-xs uppercase text-left">
            <tr className="whitespace-nowrap">
              ...
              <th className="px-3 py-3 font-semibold text-center">Tiempo Entrega</th>
            ...
            {items.map((item) => {
              ...
              return (
                <tr key={item.id} className={`border-b transition-colors ${getRowClassName(item)}`}>
                    ...
                    <td className="text-center px-3 py-4">
                      <LogisticaCotizacionSelectorModal
                        item={item}
                        onUpdated={onUpdated}
                        trigger={
                          <Button
