  ...
  >
    ...
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-500">{pedido.proyecto?.nombre || 'Sin proyecto'}</p>
      </div>
      
      {/* 💰 Información financiera */}
      <div className="text-right">
        <p className="text-2xl font-bold text-gray-900">{formatCurrency(pedido.presupuestoTotal || 0)}</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
      ...
      <div className="text-center">
        ...
        {pedido.costoRealTotal && pedido.costoRealTotal !== pedido.presupuestoTotal && (
          <p className="text-xs text-gray-600 mt-1">
              Real: {formatCurrency(pedido.costoRealTotal)}
            </p>
        )}
      </div>
...
export default function SeguimientoPedidos({ 
  ...
  const calcularMetricasPedidos = (pedidosDetalle: PedidoSeguimientoDetalle[]): MetricasPedidos => {
    ...
    const montoTotal = pedidosDetalle.reduce((sum, p) => sum + (p.presupuestoTotal || 0), 0)
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    const { result: filtered, duration } = measurePerformance('filter', () => {
      ...
      const cumpleBusqueda = busquedaLocal === '' || 
        ...
        pedido.proyecto?.nombre?.toLowerCase().includes(busquedaLocal.toLowerCase()) ||
      ...
      const cumpleMonto = (pedido.presupuestoTotal || 0) >= montoMin && (pedido.presupuestoTotal || 0) <= montoMax
